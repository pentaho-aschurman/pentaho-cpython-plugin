 ...
 */
@Step( id = "CPythonScriptExecutor", image = "pylogo.svg", name = "CPython Script Executor", description = "Executes a python script", categoryDescription = "Statistics" )
public class CPythonScriptExecutorMeta extends BaseStepMeta implements StepMetaInterface {
  ...
  @Override public void loadXML( Node stepnode, List<DatabaseMeta> dbs, IMetaStore metaStore )
    ...
    String pythonC = XMLHandler.getTagValue(stepnode, PYTHON_COMMAND);
    setPythonCommand(pythonC == null ? "" : pythonC);
    String pyPathE = XMLHandler.getTagValue(stepnode, PYTHON_PATH_ENTRIES);
    setPyPathEntries(pyPathE == null ? "" : pyPathE);
    String pyServerID = XMLHandler.getTagValue(stepnode, PYTHON_SERVER_ID);
    ...
    if ( !Utils.isEmpty( includeInputAsOutput ) ) {
    ...
    if ( !Utils.isEmpty( includeFrameRowIndex ) ) {
    ...
    if ( !Utils.isEmpty( continueOnUnset ) ) {
  ...
  @Override public void readRep( Repository rep, IMetaStore metaStore, ObjectId id_step, List<DatabaseMeta> dbs )
    ...
    setScriptToLoad( Utils.isEmpty( scriptToLoad ) ? "" : scriptToLoad ); //$NON-NLS-1$
