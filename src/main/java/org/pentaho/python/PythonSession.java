...
public class PythonSession {
  ...
  private PythonSession( String pythonCommand, String serverID, String pathEntries, boolean defaultServer,
		  LogChannelInterface log ) throws IOException {
   ...
   * @throws IOException if a problem occurs
   */
  private PythonSession( String pythonCommand, LogChannelInterface log ) throws IOException {
  ...
  public static synchronized boolean initSession( String pythonCommand, VariableSpace vars, LogChannelInterface log )
    ...
    if ( vars != null && !Utils.isEmpty( vars.getVariable( KETTLE_CPYTHON_COMMAND_PROPERTY_KEY ) ) ) {
  ...
  public static synchronized boolean initSession( String pythonCommand, String ownerID, String pathEntries,
      boolean debug, LogChannelInterface log ) throws KettleException {
    String key = pythonCommand + ( ownerID != null && ownerID.length() > 0 ? ownerID : "" );
   ...
   */
  public static String getPythonEnvCheckResults( String pythonCommand, String ownerID ) throws KettleException {
    String key = pythonCommand + ( ownerID != null && ownerID.length() > 0 ? ownerID : "" );

    if ( !m_pythonEnvCheckResults.containsKey( key ) ) {
      throw new KettleException( "The specified server/environment (" + key + ") does not seem to exist!" );
    }
